...
"""-------------------------------------------------------
Balancea un dataset haciendo muestreo con reposición de cada clase (target) desbalanceada.
Entrada:
    dataset: pandas data frame
    target: nombre del campo target en forma de texto
Salida: 
    data_balanceado: pandas data frame
-------------------------------------------------------"""
    ...
    for i in dataset[target].value_counts().index.tolist():
        ...
        if subset.shape[0] == max_frec:
            data_balanceado = pd.concat([data_balanceado, subset], axis=0)
            cant_catg += 1
...
#Funcion2: detecta valles o Picos
